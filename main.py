...
pygame.display.set_caption('Yatse')
...
# the class encapsulates a single die.
class Die:
    ...
    def select(self):
        pygame.draw.rect(self.display, RED, (self.position, self.size), 2)
...
for i in range(0, 5):
    dice_list.append(Die(window, dice_positions[i]))
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            for die in dice_list:
                if die.is_mouse_within(mouse):
                    if number_of_throws < MAX_THROWS:
                        die.select()
